...
//        private Dictionary<string, string> pluginConfig;
        ...
        public override void OnCloseGame(ICloseGameCallInfo info)
        {
            ...
            {
                this.PluginHost.LogInfo("Skipped further OnCloseGame: not succesfullyloaded.");
                info.Continue();
                return;
            }

            // Photon Voice room
            if (this.PluginHost.GameId.EndsWith("voice_"))
            {
                info.Continue();
                return;
            }
            ...
            {
                this.PluginHost.LogInfo("OnCloseGame");
                ...
                this.PluginHost.LogInfo(string.Format("Http request to {0}", info.ActorCount > 0 ? "save" : "close"));
        ...
        public override void OnCreateGame(ICreateGameCallInfo info)
        {
            ...
            {
                this.PluginHost.LogInfo("OnCreateGame: Loading");
        ...
        public override bool SetupInstance(IPluginHost host, Dictionary<string, string> config, out string errorMsg)
        {
            ...
            if (config != null && config.ContainsKey(BaseUrlKey))
            {
                ...
                if (!string.IsNullOrEmpty(this.baseUrl))
                {
                    ...
                    //host.LogInfo(this.gameCreatedUrl);
                    //host.LogInfo(this.gameClosedUrl);
                    //host.LogInfo(this.gameEventUrl);
            ...
            if (this.TryGetForwardResponse(httpResponse, out response, out errorMsg))
            {
                if (response.ResultCode == 0)
                {
                    ...
                    else
                    {
                        ...
                        this.PluginHost.LogInfo("Creating Game without loading any state - Rejoins will fail!");
                        this.SuccesfullLoaded = true;
                        callInfo.Continue();
                    }
                }
        ...
        private void PostJsonRequest(string url, WebhooksRequest forwardRequest, HttpRequestCallback callback, object userState = null, bool callAsync = false)
        {
            ...
            this.PluginHost.LogInfo(string.Format("PostJsonRequest: {0} - {1}", url, json));
        ...
        private void SaveCallback(IHttpResponse request, object userState)
        {
            ...
            {
                this.PluginHost.LogInfo(string.Format("Http request to {0} - done.", info.ActorCount > 0 ? "save" : "close"));
                info.Continue();
                return;
            }
        ...
        private bool TryGetForwardResponse(IHttpResponse httpResponse, out WebhooksResponse response, out string errorMsg)
        {
            ...
            try
            {
                if (httpResponse.Status == HttpRequestQueueResult.Success)
                {
                    ...
                    this.PluginHost.LogInfo("TryGetForwardResponse:" + responseData);
                    ...
                    if (response != null)
                    {
                        ...
                        errorMsg = string.Format(response.Message);
                    }
                    else
                    {
