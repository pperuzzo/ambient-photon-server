...
namespace Photon.Hive
{
    ...
    public class HiveGame : Room
    {
        ...
        public ObjectCacheDictionary RoomObjectsCache
        {
            get { return this.RoomState.RoomObjectsCache;  }
        }
        ...
        protected override void ExecuteOperation(HivePeer peer, OperationRequest operationRequest, SendParameters sendParameters)
        {
            try
            {
                ...
                switch ((OperationCode)operationRequest.OperationCode)
                {
                    ...
                    case OperationCode.GetObjects:
                        {
                            if (this.RoomObjectsCache.GetNumerOfObjects() > 0)
                            {
                                Log.InfoFormat("NUMBER OF OBJECTS > 0 WOW");
                                ObjectResponse response = new ObjectResponse()
                                {
                                    CurrentRoomObjects = this.RoomObjectsCache.GetAllObjects()
                                };
                                var oresponse = new OperationResponse((byte)OperationCode.GetObjects, response);
        ...
        protected bool JoinSendResponseAndEvents(HivePeer peer, JoinGameRequest joinRequest, SendParameters sendParameters, int actorNr, ProcessJoinParams prms)
        {
            ...
            //this.PublishObjectCache(peer, joinRequest);
        ...
        /// </param>
        /// <param name="joinRequest"></param>
        protected void PublishObjectCache(HivePeer litePeer, JoinGameRequest joinRequest)
        {
            var @event = new CustomEvent(0, 0, null);
            foreach (KeyValuePair<int, EventCache> entry in this.ActorEventCache)
            {
        ...
        {            
            ...
            /*
             * TODO: 
             * - ----Create RoomObjectsCacheDictionary Class
             * - ----Move process and methods to that class
             * - Make sure to serialize RoomObjectsCacheDictionary on Path close = save
             * - Deserialzie the RoomObjectsCacheDictionary on Path create = load
             * - Handle deserialized data in Unity 
             * 
             */

            // Case Update Network object: Special code for object serialization
            if (raiseEventRequest.EvCode == 2)
            {
                Log.InfoFormat(raiseEventRequest.Data.ToString());

                Hashtable data = (Hashtable)raiseEventRequest.Data;

                // We have already this object in the cache, simply update data
                if (RoomObjectsCache.TryToUpdateObject(data))
                {
                    return true;
                }
                else
                {
                    if (RoomObjectsCache.CreateNewObject(data))
                    {
                        return true;
                    } 
                    else
                    {
                        Log.WarnFormat("Couldnt create new object");
                        return false;
                    }
                }
            }

            // Case destroy network object
            if (raiseEventRequest.EvCode == 3)
            {
                Hashtable data = (Hashtable)raiseEventRequest.Data;
                RoomObjectsCache.TryRemoveObject((string)data["oid"]);
            }
