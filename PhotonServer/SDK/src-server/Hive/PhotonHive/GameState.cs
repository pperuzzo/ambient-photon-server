...
namespace Photon.Hive
{
    ...
    public class GameState 
    {
        ...
        private readonly ObjectCacheDictionary roomObjectsCache = new ObjectCacheDictionary();
        ...
        {
            get { return this.roomObjectsCache; }
        }
...
//                        this.EventCache.Slice = (int)entry.Value;
        ...
        bool SetGameStateBinaryPart(byte key, object deserializedData)
        {
            switch (key)
            {
                ...
                case 17: // ObjectCache
                {
                        var dict = deserializedData as Dictionary<string, Hashtable>;
                        if (dict != null)
                        {
                            Log.InfoFormat("We are deserializing the objects data.");
                            this.RoomObjectsCache.SetDeserializedData(dict);
                            Log.InfoFormat("Number of deserialized objects: " + RoomObjectsCache.GetNumerOfObjects());

                            //SendOperationResponse
                        }
                        else
                        {
                            Log.ErrorFormat("Can't get objects from deserialized data");
                            return false;
                        }
                    }
                    break;
            ...
            ref Dictionary<byte, ArrayList> actorGroups, out int evCount,out Dictionary<int, ArrayList> events)
        {
            ...
            Dictionary<string, Hashtable> objects = this.RoomObjectsCache.GetSerializationData();
            if (objects.Count > 0)
            {
                binary.Add("17", Serializer.SerializeBase64(objects));
            }
            return binary;
        }
